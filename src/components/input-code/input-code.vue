<template>
  <div class="vue_input_code">
    <div class="input" @click="focusInput" :style="{'height':height}">
      ...
      <span v-for="(item,index) in block" :key="index" :style="{'font-size': spanSize, 'color': spanColor, 'height': height, 'line-height': height, 'width': blockWidth}" v-text="codeArray[index] ? codeArray[index] : ''" :class="{'first':index === 0, 'last': index === number - 1}"></span>
    </div>
  </div>
</template>
